# Spec: scraper-browser-impersonation

## Purpose

Define how scrapers present a browser-consistent TLS/HTTP fingerprint (via curl_cffi Chrome impersonation), pace their requests per source, and warm up sessions so WAF-fronted sites (CloudFront on La Anónima) do not block scrape runs.
