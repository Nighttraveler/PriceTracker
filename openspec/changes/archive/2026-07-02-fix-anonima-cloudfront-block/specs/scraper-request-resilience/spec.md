...
`BaseScraper.get()` SHALL retry on 429 (rate limited) and 5xx (server error) responses up to MAX_RETRIES=3 times, waiting RETRY_BASE_WAIT * 2^attempt seconds between attempts with RETRY_BASE_WAIT=15 (15s, 30s, 60s). After exhausting retries, it SHALL raise `ScraperBlockedError` — sustained 429/5xx from a WAF-fronted site is treated as a block, not a transient failure.
...
#### Scenario: Persistent 429 raises ScraperBlockedError after MAX_RETRIES
...
- **THEN** it raises an HTTP error immediately, no retry
